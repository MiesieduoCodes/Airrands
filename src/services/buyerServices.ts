...
export const getProducts = async () => {
  try {
    ...
    const products = await Promise.all(snap.docs.map(async (doc: any) => {
        ...
        if (productData.sellerId) {
          try {
            ...
            console.error('Error fetching seller info for product:', productData.id, error);
            productData.store = 'Unknown Store';
            productData.sellerName = 'Unknown Store';
          }
        }
    ...
    // Return mock data for testing if the main collection fails
    return [
      {
        id: 'product-1',
        ...
        price: 2500,
        image: 'https://i.imgur.com/T3zF9bJ.png',
        category: 'food',
        sellerId: 'seller-1',
...
export const getRunnerAvailability = async () => {
  try {
    ...
    console.error('Error fetching runner availability:', error);
    return [];
  }
};
...
export const requestRunner = async (buyerId: string, runnerId: string) => {
  try {
    ...
    console.error('Error requesting runner:', error);
    throw error;
  }
};
  ...
  try {
    ...
    if (status === 'delivered' || status === 'completed') {
      try {
        ...
        console.error('Error creating review trigger for completed order:', error);
