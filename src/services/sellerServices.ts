  ...
  try {
    ...
    if (doc.exists) {
      ...
      return { 
        ...
        email: data?.email || '',
        phone: data?.phone || '',
        avatar: data?.avatar || data?.photoURL || data?.image || 'https://via.placeholder.com/80x80',
        coverImage: data?.coverImage || data?.bannerImage || 'https://via.placeholder.com/400x200',
